...
export function Courses({ Courses }) {
  ...
  const closePopup = () => {                                                      // hides the login succesfull popup on click close button
    document.getElementsByClassName(
      "login-successfull-popup"
    )[0].style.display = "none";
  };

  const listitems = Courses.map((value) => (                                      // takes an array of objects and return ui for single course
    <Card
      key={value.id}
      item={value}
